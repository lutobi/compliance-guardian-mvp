...
async function testOnboardingFlow() {
  ...
  try {
    ...
    const continueButton = await page.waitForSelector('button:not([disabled])', { visible: true });
    await Promise.all([
      page.waitForFunction(() => {
        const dots = document.querySelectorAll('.rounded-full');
        return dots[1].classList.contains('bg-blue-500');
      }, { timeout: 10000 }),
      continueButton.evaluate(button => button.click())
    ]);
    ...
    const createButton = await page.waitForSelector('button.flex-1:not([variant="outline"])', { visible: true });
    await Promise.all([
    ...
    const profileButton = await page.waitForSelector('button.flex-1:not([variant="outline"])', { visible: true });
    await Promise.all([
      page.waitForFunction(() => {
        const dots = document.querySelectorAll('.rounded-full');
        return dots[3].classList.contains('bg-blue-500');
      }, { timeout: 15000 }),
      profileButton.evaluate(button => button.click())
    ]);
    ...
    const finishButton = await page.waitForSelector('button.w-full', { visible: true });
    await Promise.all([
      page.waitForNavigation({ waitUntil: 'networkidle0', timeout: 15000 }),
      finishButton.evaluate(button => button.click())
    ]);
