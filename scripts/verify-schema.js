...
// Write to file
fs.writeFileSync(path.join(__dirname, 'verify-schema.sql'), sql);
console.log('Schema verification SQL written to verify-schema.sql');
console.log('\nTo verify and fix the schema:');
console.log('1. Go to https://supabase.com/dashboard/project/nrfpsbbkynykubcaarpg/sql/new');
console.log('2. Copy and paste the contents of scripts/verify-schema.sql');
console.log('3. Click "Run" to verify and fix the schema');

// Copy to clipboard
require('child_process').execSync(`cat "${path.join(__dirname, 'verify-schema.sql')}" | pbcopy`);
console.log('\nSchema verification SQL has been copied to your clipboard');
