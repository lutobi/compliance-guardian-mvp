...
async function testOnboardingFlow() {
  ...
  try {
    ...
    // Test login flow
    await page.goto('http://localhost:3009/auth/login?isNewSignIn=true');
    ...
    await page.waitForFunction(() => {
      ...
      return button && !button.disabled && createCard.classList.contains('ring-2');
    }, { timeout: 10000 });
    ...
    await page.waitForFunction(() => {
      ...
      const step = document.querySelector('span.badge');
      return form && window.getComputedStyle(form).display !== 'none' && step && step.textContent.includes('Step 2');
    }, { timeout: 10000 });
    ...
    await page.waitForFunction(() => {
      ...
      const step = document.querySelector('span.badge');
      return form && window.getComputedStyle(form).display !== 'none' && step && step.textContent.includes('Step 3');
    }, { timeout: 10000 });
    ...
    await page.waitForFunction(() => {
      ...
      const step = document.querySelector('span.badge');
      return button && window.getComputedStyle(button).display !== 'none' && step && step.textContent.includes('Step 4');
    }, { timeout: 10000 });
