...
// Complete table schema SQL with all columns
const sql = `
-- Drop and recreate workspaces table with complete schema
...
-- Drop and recreate workspace_members table with complete schema
...
// Write to file
fs.writeFileSync(path.join(__dirname, 'tables-v2.sql'), sql);
console.log('Table SQL written to tables-v2.sql');
console.log('\nTo apply the tables:');
console.log('1. Go to https://supabase.com/dashboard/project/nrfpsbbkynykubcaarpg/sql/new');
console.log('2. Copy and paste the contents of scripts/tables-v2.sql');
console.log('3. Click "Run" to recreate the tables');

// Copy to clipboard
require('child_process').execSync(`cat "${path.join(__dirname, 'tables-v2.sql')}" | pbcopy`);
