...
// Write to file
fs.writeFileSync(path.join(__dirname, 'fix-final.sql'), sql);
console.log('SQL written to fix-final.sql');
console.log('\nTo apply the fix:');
console.log('1. Go to https://supabase.com/dashboard/project/nrfpsbbkynykubcaarpg/sql/new');
console.log('2. Copy and paste the contents of scripts/fix-final.sql');
console.log('3. Click "Run" to apply all changes');

// Copy to clipboard
require('child_process').execSync(`cat "${path.join(__dirname, 'fix-final.sql')}" | pbcopy`);
