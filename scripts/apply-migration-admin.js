...
async function applyMigrations() {
  try {
    ...
    for (let i = 0; i < migrations.length; i++) {
      ...
      if (error) {
        throw new Error(`Admin API not working: ${error.message}`);
      }

      const { data, error: sqlError } = await admin.from('_migrations').select('*');
      if (sqlError) {
        throw new Error(`SQL access not working: ${sqlError.message}`);
      }

      const { error: execError } = await admin.rpc('exec_sql', { sql });
      if (execError) {
        console.error(`Failed to apply chunk ${i + 1}:`, execError.message);
        continue;
      }
