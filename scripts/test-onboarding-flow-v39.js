...
async function testOnboardingFlow() {
  ...
  try {
    ...
    page.on('request', request => {
      if (request.url().includes('/api/team/init')) {
    ...
    // Wait for workspace creation request to complete
    await page.waitForFunction(() => {
      const profileForm = document.querySelector('#profileName');
      const heading = document.querySelector('h2');
      return profileForm && heading && heading.textContent.includes('Set Up Your Profile');
    }, { timeout: 15000 });
    ...
    // Set up request interception for profile update
    let profileUpdated = false;
    page.on('request', request => {
      if (request.url().includes('/api/profile/update')) {
        profileUpdated = true;
      }
      request.continue();
    });
    ...
    // Wait for profile update request to complete
    await page.waitForFunction(() => {
      const heading = document.querySelector('h2');
      return heading && (
