...
// Write to file
fs.writeFileSync(path.join(__dirname, 'tables.sql'), sql);
console.log('Table SQL written to tables.sql');
console.log('\nTo apply the tables:');
console.log('1. Go to https://supabase.com/dashboard/project/nrfpsbbkynykubcaarpg/sql/new');
console.log('2. Copy and paste the contents of scripts/tables.sql');
console.log('3. Click "Run" to create/update the tables');

// Copy to clipboard
require('child_process').execSync(`cat "${path.join(__dirname, 'tables.sql')}" | pbcopy`);
