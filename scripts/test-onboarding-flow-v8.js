...
async function testOnboardingFlow() {
  ...
  try {
    ...
    if (currentUrl.includes('/dashboard')) {
      await page.goto('http://localhost:3008/onboarding');
    ...
    await cards[0].click(); // First card is "Create New Workspace"
    console.log('✅ Selected Create New Workspace');

    // Click Continue
    await page.waitForSelector('button:not([variant="outline"])', { timeout: 10000 });
    await page.click('button:not([variant="outline"])');
    ...
    await Promise.all([
      page.waitForResponse(
        response => response.url().includes('/api/team/init') && response.status() === 200,
        { timeout: 10000 }
      ),
      page.click('button:not([variant="outline"])')
    ]);
    console.log('✅ Submitted workspace form');

    // Wait for next step to load
    await page.waitForSelector('input#profileName', { timeout: 10000 });
    ...
    // Step 4: Completion
    await page.waitForSelector('button:not([variant="outline"])', { timeout: 10000 });
    await page.click('button:not([variant="outline"])'); // Click Go to Dashboard
