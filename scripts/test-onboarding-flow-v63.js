  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      if (createCard) {
        ...
        let fiber = Object.entries(createCard).find(([key]) => 
        ...
        if (fiber) {
          ...
          while (fiber) {
            if (fiber.memoizedProps && fiber.memoizedProps.onClick) {
              // Call React's onClick handler directly
    ...
    await page.evaluate(() => {
      ...
      if (button && !button.disabled) {
        // Get React fiber instance
        let fiber = Object.entries(button).find(([key]) => 
          key.startsWith('__reactFiber$')
        )?.[1];
    ...
    // Fill workspace form using React's event handlers
    await page.evaluate(() => {
      // Fill workspace name
      const nameInput = document.querySelector('#workspaceName');
      if (nameInput) {
        // Get React fiber instance
        let fiber = Object.entries(nameInput).find(([key]) => 
          key.startsWith('__reactFiber$')
        )?.[1];
        ...
        if (fiber) {
          ...
          while (fiber) {
            if (fiber.memoizedProps && fiber.memoizedProps.onChange) {
              ...
              fiber.memoizedProps.onChange({ target: nameInput });
      ...
      return new Promise(resolve => {
        const checkSlug = setInterval(() => {
          ...
          if (slugInput && slugInput.value === 'test-workspace') {
            ...
            if (industrySelect) {
              let fiber = Object.entries(industrySelect).find(([key]) => 
                key.startsWith('__reactFiber$')
              )?.[1];
              ...
              if (fiber) {
                while (fiber) {
                  if (fiber.memoizedProps && fiber.memoizedProps.onChange) {
                    ...
                    fiber.memoizedProps.onChange({ target: industrySelect });
            ...
            if (sizeSelect) {
              let fiber = Object.entries(sizeSelect).find(([key]) => 
                key.startsWith('__reactFiber$')
              )?.[1];
              ...
              if (fiber) {
                while (fiber) {
                  if (fiber.memoizedProps && fiber.memoizedProps.onChange) {
                    ...
                    fiber.memoizedProps.onChange({ target: sizeSelect });
      ...
      if (createButton && !createButton.disabled) {
        ...
        const handleNext = () => {
          const validateForm = () => {
            const errors = {};
            const data = {
              workspaceName: document.querySelector('#workspaceName').value,
              workspaceSlug: document.querySelector('#workspaceSlug').value,
              industry: document.querySelector('#industry').value,
              companySize: document.querySelector('#companySize').value,
              userType: 'create_workspace'
            };
          ...
          if (validateForm()) {
            // Get React fiber instance
            let fiber = Object.entries(createButton).find(([key]) => 
              key.startsWith('__reactFiber$')
            )?.[1];
    ...
    // Fill profile form using React's event handlers
    await page.evaluate(() => {
      // Fill profile name
      ...
      if (timezoneSelect) {
        let fiber = Object.entries(timezoneSelect).find(([key]) => 
          key.startsWith('__reactFiber$')
        )?.[1];
        ...
        if (fiber) {
          while (fiber) {
            if (fiber.memoizedProps && fiber.memoizedProps.onChange) {
              ...
              fiber.memoizedProps.onChange({ target: timezoneSelect });
    ...
    await page.evaluate(() => {
      ...
      if (continueButton && !continueButton.disabled) {
        let fiber = Object.entries(continueButton).find(([key]) => 
          key.startsWith('__reactFiber$')
        )?.[1];
