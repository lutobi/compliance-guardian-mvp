...
// Write to file
fs.writeFileSync(path.join(__dirname, 'fix-schema.sql'), sql);
console.log('Schema fix SQL written to fix-schema.sql');
console.log('\nTo fix the schema:');
console.log('1. Go to https://supabase.com/dashboard/project/nrfpsbbkynykubcaarpg/sql/new');
console.log('2. Copy and paste the contents of scripts/fix-schema.sql');
console.log('3. Click "Run" to fix the schema');

// Copy to clipboard
require('child_process').execSync(`cat "${path.join(__dirname, 'fix-schema.sql')}" | pbcopy`);
console.log('\nSchema fix SQL has been copied to your clipboard');
