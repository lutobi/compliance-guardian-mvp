...
async function testOnboardingFlow() {
  ...
  try {
    ...
    // Wait for workspace creation API call to complete
    ...
    // Click submit profile and wait for request
    await page.evaluate(() => {
      const continueButton = document.querySelector('button.flex-1');
    ...
    // Click finish and wait for API call and redirect
    await page.evaluate(() => {
      const button = document.querySelector('button.w-full');
      if (button && !button.disabled) {
        button.click();
    ...
    // Wait for team init API call and redirect
