...
async function testOnboardingFlow() {
  ...
  try {
    ...
    await page.evaluate(() => {
      // Helper to create React synthetic events
      function createSyntheticEvent(type, data = {}) {
      ...
      if (createCard) {
        // Trigger React's onClick handler
    ...
    await page.evaluate(() => {
      ...
      if (nameInput) {
        ...
        const changeEvent = createSyntheticEvent('change', {
          target: nameInput,
      ...
      return new Promise(resolve => {
        const checkSlug = setInterval(() => {
          ...
          if (slugInput && slugInput.value === 'test-workspace') {
            ...
            if (industrySelect) {
              const industryEvent = createSyntheticEvent('change', {
                target: industrySelect,
                currentTarget: industrySelect,
                value: 'technology'
              });
              industrySelect.value = 'technology';
              industrySelect.dispatchEvent(industryEvent);
            }
            
            // Fill company size
            const sizeSelect = document.querySelector('#companySize');
            if (sizeSelect) {
              const sizeEvent = createSyntheticEvent('change', {
                target: sizeSelect,
                currentTarget: sizeSelect,
                value: '1-10'
              });
