...
async function testOnboardingFlow() {
  ...
  try {
    ...
    // Set up state tracking for workspace creation
    await page.evaluate(() => {
      ...
      const observer = new MutationObserver((mutations) => {
        const heading = document.querySelector('h2');
        if (heading && heading.textContent.includes('Set Up Your Profile')) {
    ...
    // Wait for workspace creation and step transition
    await page.waitForFunction(() => {
    ...
    // Set up state tracking for profile submission
    await page.evaluate(() => {
    ...
    // Wait for profile submission and step transition
    await page.waitForFunction(() => {
