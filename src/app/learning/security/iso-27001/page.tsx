...
import { iso27001Learning } from '@/data/learning/frameworks/iso-27001';
import Link from 'next/link';
import { ExternalLink, ChevronRight, Book, Wrench, FileText, Shield, AlertTriangle } from 'lucide-react';
import { CollapsibleSection } from '@/components/CollapsibleSection';

export const metadata: Metadata = {
  title: `${iso27001Learning.name} - Learning Hub`,
  description: iso27001Learning.overview.description,
};

export default function ISO27001Learning() {
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center text-sm text-gray-600 mb-8">
        ...
        <ChevronRight className="w-4 h-4 mx-2" />
        <span className="text-gray-900">ISO 27001</span>
      </div>

      {/* Header */}
      <div className="mb-12">
        <h1 className="text-4xl font-bold mb-4">{iso27001Learning.name}</h1>
        <p className="text-xl text-gray-600 max-w-4xl">
          {iso27001Learning.overview.description}
        </p>
      </div>
      ...
      <CollapsibleSection title="Overview" defaultOpen={true} id="overview">
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            ...
            <p className="text-gray-600 mb-4">{iso27001Learning.overview.importance}</p>
            <h4 className="font-semibold mb-2">Who Needs to Comply?</h4>
            <ul className="list-disc list-inside text-gray-600 space-y-2">
              {iso27001Learning.overview.applicability.map((item, index) => (
          ...
          <div>
            ...
            <ul className="space-y-3">
              {iso27001Learning.overview.benefits.map((benefit, index) => (
      ...
      <CollapsibleSection title="Key Components" id="components">
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-6">
          {iso27001Learning.keyComponents.map((component, index) => (
      ...
      <CollapsibleSection title="Implementation Guide" id="implementation">
        <div className="space-y-6">
          {iso27001Learning.implementation?.steps?.map((step, index) => (
            <div key={index} className="bg-gray-50 rounded-lg p-6">
      ...
      <CollapsibleSection title="Common Challenges" id="challenges">
        <div className="grid md:grid-cols-2 gap-6">
          {iso27001Learning.implementation?.commonChallenges?.map((item, index) => (
            <div key={index} className="bg-gray-50 rounded-lg p-6">
      ...
      <CollapsibleSection title="Controls & Implementation" id="controls">
        {iso27001Learning.controls.map((control, index) => (
      ...
      <CollapsibleSection title="Resources & Links" id="resources">
        ...
        <div className="bg-gray-50 rounded-lg p-6 mb-6">
          ...
          <div className="mb-4">
            <p className="text-gray-600">{iso27001Learning.governingBody.description}</p>
          </div>
          <div className="flex items-center text-blue-600 hover:text-blue-700">
            <ExternalLink className="w-4 h-4 mr-2" />
            <a
              href={iso27001Learning.governingBody.website}
              target="_blank"
              rel="noopener noreferrer"
              className="text-blue-600 hover:underline"
            >
              Visit {iso27001Learning.governingBody.name}
            </a>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {iso27001Learning.governingBody.resources.map((resource, index) => (
