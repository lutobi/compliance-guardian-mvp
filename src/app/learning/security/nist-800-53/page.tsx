...
import { nist80053Learning } from '@/data/learning/frameworks/nist-800-53';
import Link from 'next/link';
import { ExternalLink, ChevronRight, Book, Wrench, FileText, Shield, AlertTriangle } from 'lucide-react';
import { CollapsibleSection } from '@/components/CollapsibleSection';

export const metadata: Metadata = {
  title: `${nist80053Learning.name} - Learning Hub`,
  description: nist80053Learning.overview.description,
};

export default function NIST80053Learning() {
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center text-sm text-gray-600 mb-8">
        ...
        <Link href="/learning/security" className="hover:text-blue-600">Security</Link>
        <ChevronRight className="w-4 h-4 mx-2" />
        <span className="text-gray-900">NIST 800-53</span>
      </div>

      {/* Header */}
      <div className="mb-12">
        <h1 className="text-4xl font-bold mb-4">{nist80053Learning.name}</h1>
        <p className="text-xl text-gray-600 max-w-4xl">
          {nist80053Learning.overview.description}
        </p>
      </div>
      ...
      <CollapsibleSection title="Overview" defaultOpen={true} id="overview">
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            ...
            <p className="text-gray-600 mb-4">{nist80053Learning.overview.importance}</p>
            <h4 className="font-semibold mb-2">Who Needs to Comply?</h4>
            <ul className="list-disc list-inside text-gray-600 space-y-2">
              {nist80053Learning.overview.applicability.map((item, index) => (
          ...
          <div>
            ...
            <ul className="space-y-3">
              {nist80053Learning.overview.benefits.map((benefit, index) => (
      ...
      <CollapsibleSection title="Key Components" id="components">
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-6">
          {nist80053Learning.keyComponents.map((component, index) => (
      ...
      <CollapsibleSection title="Implementation Guide" id="implementation">
        <div className="space-y-6">
          {nist80053Learning.implementation?.steps?.map((step, index) => (
            <div key={index} className="bg-gray-50 rounded-lg p-6">
      ...
      <CollapsibleSection title="Common Challenges" id="challenges">
        <div className="grid md:grid-cols-2 gap-6">
          {nist80053Learning.implementation?.commonChallenges?.map((item, index) => (
            <div key={index} className="bg-gray-50 rounded-lg p-6">
      ...
      <CollapsibleSection title="Controls & Implementation" id="controls">
        {nist80053Learning.controls.map((control, index) => (
      ...
      <CollapsibleSection title="Resources & Links" id="resources">
        ...
        <div className="bg-gray-50 rounded-lg p-6 mb-6">
          ...
          <div className="mb-4">
            <p className="text-gray-600">{nist80053Learning.governingBody.description}</p>
          </div>
          <div className="flex items-center text-blue-600 hover:text-blue-700">
            <ExternalLink className="w-4 h-4 mr-2" />
            <a
              href={nist80053Learning.governingBody.website}
              target="_blank"
              rel="noopener noreferrer"
              className="text-blue-600 hover:underline"
            >
              Visit {nist80053Learning.governingBody.name}
            </a>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {nist80053Learning.governingBody.resources.map((resource, index) => (
